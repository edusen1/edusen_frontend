...
type AbsItem = {
  id: string; enseignantId: string; enseignantNom: string; enseignantSpecialite: string | null;
...
export default function AbsencesEnseignantsPage() {
  ...
  const fetchAbsences = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiClient.get('/admin/absences-enseignants');
      const data = (Array.isArray(res.data) ? res.data : []) as R[];
      setAbsences(data.map((a) => {
        const ens = (a.enseignant ?? {}) as R;
        return {
          id: String(a.id ?? ''), enseignantId: String(a.enseignantId ?? ''),
          enseignantNom: `${ens.firstName ?? ''} ${ens.lastName ?? ''}`.trim(),
          enseignantSpecialite: (ens.specialite as string) ?? null,
  ...
  async function handleRejeter(id: string) {
    try { await apiClient.patch(`/admin/absences-enseignants/${id}/rejeter`); toast.success('Rejetée'); setAbsences((p) => p.map((a) => a.id === id ? { ...a, statut: 'REJETEE' } : a)); } catch { toast.error('Erreur'); }
  }
  ...
  const topAbsents = useMemo(() => {
    ...
    for (const a of absences) {
      const existing = map.get(a.enseignantId);
      if (existing) existing.count++;
      else map.set(a.enseignantId, { nom: a.enseignantNom, specialite: a.enseignantSpecialite, count: 1 });
    }
  ...
  const filtered = useMemo(() => absences.filter((a) => {
    ...
    if (search) { const q = search.toLowerCase(); if (!a.enseignantNom.toLowerCase().includes(q) && !a.motif.toLowerCase().includes(q)) return false; }
    return true;
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map((a) => [a.enseignantNom, a.enseignantSpecialite ?? '', new Date(a.dateDebut).toLocaleDateString('fr-FR'), new Date(a.dateFin).toLocaleDateString('fr-FR'), TYPE_MAP[a.typeAbsence]?.label ?? a.typeAbsence, STATUT_MAP[a.statut]?.label ?? a.statut, (a.motif ?? '').replace(/,/g, ';')]);
    const csv = [headers, ...rows].map((r) => r.map((v) => `"${v}"`).join(',')).join('\n');
    const blob = new Blob(['\ufeff' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `absences-enseignants_${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f5f7fa' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e6ebf1', flexShrink: 0 }}>
        <div style={{ height: 62, display: 'flex', alignItems: 'center', padding: '0 28px', gap: 14 }}>
          <div style={{ fontSize: 17, fontWeight: 700, color: '#0f172a' }}>Absences enseignants</div>
      ...
      {section === 'dashboard' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 28px 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #e6ebf1', padding: '18px 20px' }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', marginBottom: 14 }}>Top 10 enseignants les plus absents</div>
        ...
        <>
          ...
          <div style={{ flexShrink: 0, padding: '12px 28px 0', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <div style={{ flex: 1, maxWidth: 260, display: 'flex', alignItems: 'center', gap: 8, border: '1px solid #d9e0e8', background: '#fff', padding: '0 12px' }}>
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher enseignant…" style={{ border: 'none', outline: 'none', fontSize: 13, height: 36, background: 'transparent', fontFamily: 'inherit', width: '100%' }} />
            </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 28px 28px' }}>
            <div style={{ background: '#fff', border: '1px solid #e6ebf1' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 90px 100px 140px', padding: '11px 18px', background: '#f8fafc', borderBottom: '1px solid #e6ebf1' }}>
                {['Enseignant', 'Date début', 'Date fin', 'Type', 'Statut', 'Actions'].map((h) => (
              ...
              {filtered.map((a, idx) => {
                ...
                return (
                  <div key={a.id} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 90px 100px 140px', padding: '10px 18px', borderBottom: idx < filtered.length - 1 ? '1px solid #eef2f6' : 'none', alignItems: 'center', cursor: 'pointer' }}
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{a.enseignantNom}</div>
                      {a.enseignantSpecialite && <div style={{ fontSize: 10, color: '#94a3b8' }}>{a.enseignantSpecialite}</div>}
                    </div>
      ...
      {section === 'demandes' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 28px 28px' }}>
          <div style={{ fontSize: 13, color: '#64748b', marginBottom: 14 }}>
            Demandes d&apos;absence déclarées par les enseignants — nécessitent votre validation.
          </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <span style={{ fontSize: 15, fontWeight: 700, color: '#0f172a' }}>{d.enseignantNom}</span>
                        {d.enseignantSpecialite && <span style={{ fontSize: 12, color: '#64748b' }}>{d.enseignantSpecialite}</span>}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex' }}>
          ...
          <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 480, background: '#fff', boxShadow: '-4px 0 20px rgba(0,0,0,.1)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #e6ebf1', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#0f172a' }}>{selectedAbsence.enseignantNom}</div>
                {selectedAbsence.enseignantSpecialite && <div style={{ fontSize: 12, color: '#64748b' }}>{selectedAbsence.enseignantSpecialite}</div>}
              </div>
