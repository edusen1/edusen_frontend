...
export default function RhProfilPage() {
  ...
  const [infoForm, setInfoForm] = useState({
    firstName: (user as Record<string, unknown>)?.firstName as string ?? user?.prenom ?? '',
    lastName: (user as Record<string, unknown>)?.lastName as string ?? user?.nom ?? '',
    email: user?.email ?? '',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f5f7fa' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e6ebf1', minHeight: 62, flexShrink: 0, display: 'flex', alignItems: 'center', padding: '0 28px', flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#64748b' }}>Ressources Humaines</div>
        </div>
      </div>
